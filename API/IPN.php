...
class IPN
{
    ...
    public function makeApiCall($path, $args, $formId, $method = 'GET')
    {
        ...
        if ($method == 'POST') {
            $response = wp_remote_post('https://api.authorizedotnet.co/v2/' . $path, [
                'headers' => $headers,
                'body' => json_encode($args)
            ]);
        } else {
            $response = wp_remote_get('https://api.authorizedotnet.co/v2/' . $path, [
                'headers' => $headers,
                'body' => $args
            ]);
        }
        ...
        if (empty($responseData['id'])) {
            ...
            if (!$message) {
                $message = 'Unknown AuthorizeDotNet API request error';
            }
